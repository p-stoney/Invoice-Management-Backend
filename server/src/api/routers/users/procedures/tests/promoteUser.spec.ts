...
describe('promoteUser procedure', () => {
  ...
  it('successfully promotes a user', async () => {
    ...
    testdb.user.update.mockResolvedValue({ ...mockUserBeforePromotion, role: Role.ADMIN });

    await promoteUser(promoteUserInput, mockCtx);

    const updatedUser = await testdb.user.findUnique({ where: { id: promoteUserInput.userId } });
  ...
  it('throws NOT_FOUND error when a user does not exist', async () => {
    ...
    await expect(promoteUser(promoteUserInput, mockCtx))
      .rejects
  ...
  it('throws FORBIDDEN error when a user tries to promote a user with a higher role', async () => {
    ...
    await expect(promoteUser(promoteUserInput, mockCtx))
      .rejects
      .toMatchObject({ code: 'BAD_REQUEST', message: 'User already has admin privileges.' });
  });
});