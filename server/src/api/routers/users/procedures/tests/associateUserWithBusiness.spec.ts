...
describe('associateUserWithBusiness procedure', () => {
    ...
    it('successfully associates a user with businesses', async () => {
        ...
        testdb.business.findMany.mockResolvedValueOnce(associateUserInput.businessIds.map(id => ({
                id,
                userId: 2,
                name: '',
                createdAt: new Date(),
        ...
        expect(testdb.user.update).toHaveBeenCalledWith(expect.objectContaining({
            where: { id: associateUserInput.userId },
            data: expect.objectContaining({
                businesses: { connect: associateUserInput.businessIds.map(id => ({ id })) },
            }),
        }));
    });
    ...
    it('throws NOT_FOUND error for non-existent user', async () => {
      ...
      await expect(associateUserWithBusiness(associateUserInput, mockCtx))
        .rejects
    ...
    it('throws NOT_FOUND error for one or more non-existent businesses', async () => {
        ...
        await expect(associateUserWithBusiness(associateUserInput, mockCtx))
            .rejects
            .toThrowError(new TRPCError({ code: 'NOT_FOUND', message: 'One or more businesses not found' }));
    });
