...
describe('demoteUser procedure', () => {
  ...
  it('successfully demotes a user', async () => {
    ...
    testdb.user.update.mockResolvedValue({ ...mockUserBeforeDemotion, role: Role.USER });

    await demoteUser(demoteUserInput, mockCtx);

    const updatedUser = await testdb.user.findUnique({ where: { id: demoteUserInput.userId } });
  ...
  it('throws NOT_FOUND error when a user does not exist', async () => {
    ...
    await expect(demoteUser(demoteUserInput, mockCtx))
      .rejects
  ...
  it('throws FORBIDDEN error when a user tries to demote a user with a higher role', async () => {
    ...
    await expect(demoteUser(demoteUserInput, mockCtx))
      .rejects
      .toMatchObject({ code: 'FORBIDDEN', message: 'SuperAdmin users cannot be demoted.' });
  });
});