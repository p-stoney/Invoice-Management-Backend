...
describe('createInvoice procedure', () => {
  ...
  beforeEach(() => {
    ...
    testdb.invoice.create.mockResolvedValue({
        ...
        updatedAt: new Date(),
        deletedAt: null,
    } as Invoice & { items: { productId: number; quantity: number; price: string }[] });

    vitest.mocked(fetchProductPrices).mockResolvedValue(new Map([
      [1, '100.00'],
      [2, '200.00'],
    ]));
  });
  ...
  it('successfully creates a new invoice', async () => {
    ...
    expect(result).toMatchObject({
      ...
      items: expect.arrayContaining([
        expect.objectContaining({ productId: 1, quantity: 10, price: '100.00' }),
  ...
  it('throws NOT_FOUND error when distributor does not exist', async () => {
    ...
    await expect(createInvoice(createInvoiceInput, mockCtx))
      .rejects
      .toMatchObject({ code: 'NOT_FOUND', message: `Distributor with ID ${createInvoiceInput.distributorId} not found.` });
  });
});